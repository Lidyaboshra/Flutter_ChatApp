...
def get_bot_reponse():
    d={}
    userText =str( request.args['msg'])
    if(userText=='quit' or userText== 'q'):
        return "Hope I was able to help you today! Take care, bye!"
    answer=str(evaluateInput(encoder, decoder, searcher, voc,userText))
    d['output']=answer
    full_str = ' '.join([str(elem) for elem in  d['output']])
    print("this full"+ full_str)
    print("this join"+" ".join( d['output']))
    print( d['output'])
    stri  =json.dumps( d['output'])
    listToStr = ' '.join([str(elem) for elem in stri])
    print(listToStr)
